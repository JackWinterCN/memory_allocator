...
public:
  ...
  public:
    ...
    static std::shared_ptr<Allocator> createMmap(const char *dirName,
                                                 const char *prefix,
                                                 const char *posfix,
...
public:
  ...
  EagerBufferAllocator(std::shared_ptr<Allocator> parent,
                       size_t align = MNN_MEMORY_ALIGN_DEFAULT,
                       size_t minAllocSize = 0)
      : mAllocator(parent), mAlign(align), mMinAllocSize(minAllocSize) {
    // nothing to do
  }
  /**
...
public:
  /**
   * @brief alloc CHUNK pointer with given size. if any reusable pointer matches
   * size, reuse it.
...
public:
  DeferBufferAllocator(SingleBufferWithAllocator *parent,
                       size_t align = MNN_MEMORY_ALIGN_DEFAULT,
