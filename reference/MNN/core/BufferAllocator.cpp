...
namespace MNN {
// MemChunk function
bool MemChunk::invalid() const { return mNode == nullptr && first == nullptr; }
...
ErrorCode BufferAllocator::compute() { return NO_ERROR; }
ErrorCode BufferAllocator::apply() { return NO_ERROR; }
...
public:
  MmapAllocator(const char *dirName, const char *prefix, const char *posfix,
                bool autoRemove) {
    if (nullptr != dirName) {
      mFileName = dirName;
...
public:
  RecurseAllocator(BufferAllocator *parent) { mParent = parent; }
  ...
  virtual void onRelease(MemChunk chunk) override { mParent->free(chunk); }
...
BufferAllocator::Allocator::createDefault() {
  std::shared_ptr<BufferAllocator::Allocator> _res;
  _res.reset(new DefaultAllocator);
  return _res;
}
std::shared_ptr<BufferAllocator::Allocator>
BufferAllocator::Allocator::createMmap(const char *dirName, const char *prefix,
                                       const char *posfix, bool autoRemove) {
  std::shared_ptr<BufferAllocator::Allocator> _res;
...
std::shared_ptr<BufferAllocator::Allocator>
BufferAllocator::Allocator::createRecurse(BufferAllocator *parent) {
  std::shared_ptr<BufferAllocator::Allocator> _res;
...
void EagerBufferAllocator::returnMemory(FREELIST *listP, SharedPtr<Node> node,
                                        bool permitMerge) {
  auto &list = *listP;
...
void EagerBufferAllocator::barrierBegin() { MNN_ASSERT(mGroups.empty()); }
...
void EagerBufferAllocator::endGroup() { mCurrentFreeList = nullptr; }

void EagerBufferAllocator::sync() { mAllocator->sync(); }

std::pair<void *, size_t>
EagerBufferAllocator::getFromFreeList(FREELIST *list, size_t size,
...
SingleBufferWithAllocator::~SingleBufferWithAllocator() { release(); }
...
DeferBufferAllocator::DeferBufferAllocator(SingleBufferWithAllocator *root,
                                           size_t align,
                                           MemChunkApplyToTensor func)
    : mAlign(align) {
  if (nullptr == func) {
