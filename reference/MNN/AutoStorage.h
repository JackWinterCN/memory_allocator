...
public:
  ...
  AutoStorage(int size) {
    mData =
        (T *)MNNMemoryAllocAlign(sizeof(T) * size, MNN_MEMORY_ALIGN_DEFAULT);
    mSize = size;
  }
  /**
   * @brief deinitializer.
   ...
   * @param size  number of elements.
   * @warning do NOT call `free` or `MNNMemoryFreeAlign` for data pointer passes
   * in.
   */
  ...
  /**
   * @brief set data with outter pointer, can decide whether free pointer when
   * destructor
