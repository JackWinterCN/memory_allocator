...
/**
 * data container.
 * data for host tensor is saved in `host` field. its memory is allocated malloc
 * directly. data for device tensor is saved in `deviceId` field. its memory is
 * allocated by session's backend. usually, device tensors are created by engine
 * (like net, session). meanwhile, host tensors could be created by engine or
 * user.
 */
...
public:
  /**
   * @brief create a tensor with dimension size and type without acquire memory
   * for data.
   ...
   */
  Tensor(const Tensor *tensor, DimensionType type = CAFFE,
...
public:
   ...
   * @warning memory for data won't be acquired. call backend's onAcquireBuffer
   * to get memory ready.
   */
  static Tensor *createDevice(const std::vector<int> &shape, halide_type_t type,
                              DimensionType dimType = TENSORFLOW);

  /**
   * @brief create tensor with shape and dimension type. data type is
   * represented by `T`.
   * @param shape     tensor shape.
   * @param dimType   dimension type.
   * @return created tensor.
   ...
   */
  template <typename T>
  static Tensor *createDevice(const std::vector<int> &shape,
                              DimensionType dimType = TENSORFLOW) {
  ...
  static Tensor *create(const std::vector<int> &shape, halide_type_t type,
                        void *data = NULL, DimensionType dimType = TENSORFLOW);

  /**
   * @brief create tensor with shape, data and dimension type. data type is
   * represented by `T`.
   * @param shape     tensor shape.
   * @param data      data to save.
   * @param dimType   dimension type.
   ...
   */
  template <typename T>
  static Tensor *create(const std::vector<int> &shape, void *data = NULL,
                        DimensionType dimType = TENSORFLOW) {
   ...
   * @brief copy tensor.
   * @param src     tensor
   * @param deepCopy whether create new content and copy, currently only support
   * deepCopy = false
   */
...
public:
   ...
   */
  static Tensor *createHostTensorFromDevice(const Tensor *deviceTensor,
                                            bool copyData = true);

public:
  const halide_buffer_t &buffer() const { return mBuffer; }
  halide_buffer_t &buffer() { return mBuffer; }
   ...
   * @return data type.
   */
  inline halide_type_t getType() const { return mBuffer.type; }

  /**
   * @brief visit host memory, data type is represented by `T`.
   * @return data point in `T` type.
   */
  template <typename T> T *host() const { return (T *)mBuffer.host; }

  /**
   * @brief visit device memory.
   * @return device data ID. what the ID means varies between backends.
   */
  uint64_t deviceId() const { return mBuffer.device; }

public:
  int dimensions() const { return mBuffer.dimensions; }
  ...
  /**
   * @brief calculate number of bytes needed to store data taking reordering
   * flag into account.
  ...
  /**
   * @brief calculate number of elements needed to store data taking reordering
   * flag into account.
   * @return elements needed to store data
   */
  inline int elementSize() const { return size() / mBuffer.type.bytes(); }
...
public:
  ...
  inline int batch() const { return mBuffer.dim[0].extent; }

  // visit dimension's extent & stride
  inline int stride(int index) const { return mBuffer.dim[index].stride; }
  inline int length(int index) const { return mBuffer.dim[index].extent; }
  ...
  /**
   * @brief For GPU and Other Device, get memory directly, see MNNSharedContext
   * for detail
   * @return Success or not. If type != tensor's backend's type or type is cpu ,
   * return false
   */
