...
file_t MNNCreateFile(const char *file_name) {
#if defined(WIN32) || defined(_WIN32) || defined(_WIN64) || defined(_MSC_VER)
  HANDLE hd =
      CreateFile(file_name,                    // File Name
  ...
  int fd = open(
      file_name, // File Name
      O_RDWR | O_CREAT |
          O_TRUNC, // Read and Write and Create the file and cover existing file
...
file_t MNNOpenFile(const char *file_name, uint32_t flags) {
  ...
  HANDLE hd = CreateFile(file_name,             // File Name
                         mode,                  // Opening Mode
                         0,                     // No Sharing
                         NULL,                  // No Security
...
void *MNNMmapFile(file_t file, size_t size) {
...
#if defined(WIN32) || defined(_WIN32) || defined(_WIN64) || defined(_MSC_VER)
  HANDLE hFileMapping =
      CreateFileMapping(file, NULL, PAGE_READWRITE, (size >> 32) & 0xffffffff,
  ...
  void *addr =
      MapViewOfFile(hFileMapping, FILE_MAP_READ | FILE_MAP_WRITE, 0, 0, size);
